...
if [ ! -f $ARCHIVEPATH ]; then
    if ! wget -q --show-progress --progress=bar --no-check-certificate --no-cache --no-cookies -O "$ARCHIVEPATH" http://rgsretro1986.ds78102.seedhost.eu/update/v43/fullinstall.tar.gz; then
...
echo ""
echo -e "#####################################################################################"
echo -e "# Shutting down emulationstation to avoid files locks in /userdata/.....            #"
echo -e "# The monitor screen will shutdown, this is normal. Your computer is still running !#"
echo -e "#####################################################################################"
echo ""
sleep 10
